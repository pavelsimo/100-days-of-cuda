...
__global__ void finalize_integration(float* result, float c) {
    *result *= c;
}
...
extern "C" void solve(const float* y_samples, float* result, float a, float b, int n_samples) {
    ...
    monte_carlo_integration<<<blocks, threads>>>(y_samples, result, a, b, n_samples);
    float c = 1.0 * (b - a) / n_samples;
    finalize_integration<<<1, 1>>>(result, c);
