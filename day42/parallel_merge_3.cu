...
__global__ void mergeA(const float *A, const float *B, float *C, int M, int N) {
...
__global__ void mergeB(const float *A, const float *B, float *C, int M, int N) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i < N) {
        int idx = i + upper_bound(A, B[i], M);
        C[idx] = B[i];
    }
}
...
extern "C" void solve(const float* A, const float* B, float* C, int M, int N) {
    ...
    // launch each on its own non-blocking stream
    cudaStream_t streamA, streamB;
    cudaStreamCreateWithFlags(&streamA, cudaStreamNonBlocking);
    cudaStreamCreateWithFlags(&streamB, cudaStreamNonBlocking);

    mergeA<<<(M + threads - 1) / threads, threads, 0, streamA>>>(A, B, C, M, N);
    mergeB<<<(N + threads - 1) / threads, threads, 0, streamB>>>(A, B, C, M, N);
