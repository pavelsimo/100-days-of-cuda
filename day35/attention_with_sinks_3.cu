...
__device__ float block_reduce_max(float value) {
    ...
    __syncthreads();
    if (warp == 0) {
        value = (lane < blockDim.x / warpSize) ? t[lane] : -FLT_MAX;
        value = warp_reduce_max(value);
        if (lane == 0) {
            t[0] = value;
        }
...
__device__ float block_reduce_sum(float value) {
    ...
    __syncthreads();
    if (warp == 0) {
        value = (lane < blockDim.x / warpSize) ? t[lane] : 0.0f;
        value = warp_reduce_sum(value);
        if (lane == 0) {
            t[0] = value;
        }
...
__global__ void softmax(float* scores, int M, int N, int num_sinks, int window_size) {
    ...
    const float rowMax = block_reduce_max(threadMax);
    ...
    threadSum = block_reduce_sum(threadSum);
...
extern "C" void solve(const float* Q, const float* K, const float* V, float* output, int M, int d,
    ...
    softmax<<<M, BLOCK>>>(scores, M, M, num_sinks, window_size);
